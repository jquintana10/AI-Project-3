...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for accion in self.all_actions:

            accion_node = PgNode_a(accion)

            if accion_node.prenodes.issubset(self.s_levels[level]):
                for state_node in accion_node.prenodes:
                    for literal_nodos in self.s_levels[level]:

                        if (literal_nodos.symbol == state_node.symbol) and (literal_nodos.is_pos == state_node.is_pos):
                            accion_node.parents.add(literal_nodos)
                            literal_nodos.children.add(accion_node)
                            self.a_levels[level].add(accion_node)
    ...
    def add_literal_level(self, level):
        ...
        for accion_nodos in self.a_levels[level-1]:
            for effect_nodos in accion_nodos.effnodes:
                literal_node = PgNode_s(effect_nodos.symbol, effect_nodos.is_pos)
                literal_node.parents.add(accion_nodos)
                accion_nodos.children.add(literal_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_eff in node_a1.action.effect_add:
            ...
            if pos_eff in node_a2.action.effect_rem:
                #print("Paso:", pos_eff)
                inconsistent = True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pre_nodos_a1 in node_a1.parents:
            for pre_nodos_a2 in node_a2.parents:
                if pre_nodos_a1.is_mutex(pre_nodos_a2):
                    competing = True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for accion_s1 in node_s1.parents:
            for accion_s2 in node_s2.parents:
                if not accion_s1.is_mutex(accion_s2):
